...
public class RefreshCachedSupplier<T> implements AutoCloseable {
    ...
    private RefreshResult<T> handleFetchedSuccess(RefreshResult<T> value) {
        ...
        if (now < value.staleTime() + STALE_TIME) {
            logger.warning("Retrieved value stale time is %s in the past ({}). Using staleTime "
        ...
        if (now < this.cachedValue.staleTime()) {
            ...
            switch (staleValueBehavior) {
                case STRICT:
                    // 立马重试
                    logger.warning("Cached value expiration is in the past (" + value.staleTime() + "). Using expiration "
                            + "of " + (now + 1000));
    ...
    private RefreshResult<T> handleFetchedFailure(Exception exception) throws Exception {
        logger.verbose("Refresh credentials failed, cached value is {}", cachedValue);
    ...
    private long maxStaleFailureJitter(int numFailures) {
